...
[TestClass]
public sealed class KeyGestureConverterTest : TypeConverterTestBase
{
    ...
    public void ConvertFrom_ReturnsExpected(KeyGesture expected, ITypeDescriptorContext context, CultureInfo cultureInfo, string value)
    {
        ...
        Assert.AreEqual(expected.Key, converted.Key);
        Assert.AreEqual(expected.Modifiers, converted.Modifiers);
        Assert.AreEqual(expected.DisplayString, converted.DisplayString);
    }
