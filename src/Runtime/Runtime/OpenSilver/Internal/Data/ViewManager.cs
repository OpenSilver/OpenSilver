...
internal readonly struct WeakRefKey
{
    //------------------------------------------------------
    //
    //  Constructors
    ...
    internal WeakRefKey(object target, bool useWeakReference)
    {
        _useWeakReference = useWeakReference;
        _target = useWeakReference ? new WeakReference<object>(target) : target;
    ...
    internal object Target
    {
        get
        {
            ...
            {
                var weakRef = (WeakReference<object>)_target;
                weakRef.TryGetTarget(out object target);
                return target;
            }
            return _target;
        }
    ...
    public override bool Equals(object o) => o is WeakRefKey ck && Equals(ck);

    public bool Equals(WeakRefKey ck)
    {
        object c1 = Target;
        object c2 = ck.Target;
        ...
        {
            return _target == ck._target;
        }
    }

    // overload operator for ==, to be same as Equal implementation.
    public static bool operator ==(WeakRefKey left, WeakRefKey right) => left.Equals(right);
...
internal sealed class ViewTable : Dictionary<WeakRefKey, ViewRecord>
{
    //------------------------------------------------------
    //
    //  Internal Properties
    ...
    internal ViewRecord this[CollectionViewSource cvs]
    {
        get
        {
            TryGetValue(new WeakRefKey(cvs, false), out ViewRecord vr);
            return vr;
        }
        set
        {
            Debug.Assert(cvs != null, "Required CVS key is missing");
            base[new WeakRefKey(cvs, true)] = value;
        }
    }
    ...
    internal bool Purge()
    {
        var al = new List<WeakRefKey>();
        foreach (var de in this)
        {
            WeakRefKey key = de.Key;
            if (key.Target == null)
            {
                ViewRecord vr = de.Value;

                if (vr.View is CollectionView cv)
                {
                    if (!cv.IsInUse)
                    {
...
internal sealed class CollectionRecord
{
    public ViewTable ViewTable
    {
        get
        {
            _wrViewTable.TryGetTarget(out ViewTable target);
            return target;
        }
        set { _wrViewTable = new WeakReference<ViewTable>(value); }
    }

    public bool IsAlive => SynchronizationInfo.IsAlive || _wrViewTable.TryGetTarget(out _);

    public SynchronizationInfo SynchronizationInfo;

    private WeakReference<ViewTable> _wrViewTable = NullWeakRef;

    private static readonly WeakReference<ViewTable> NullWeakRef = new(null);
}

internal readonly struct SynchronizationInfo
{
    public SynchronizationInfo(object context, CollectionSynchronizationCallback callback)
    {
        ...
        else
        {
            ...
            _context = new WeakReference<object>(context);
            _callbackMethod = callback.Method;

            // distinguish static methods (target = null) from methods whose
            // target gets GC'd
            object target = callback.Target;
            _callbackTarget = target is not null ? new WeakReference<object>(target) : StaticWeakRef;
        }
    }

    public bool IsSynchronized => _context is not null || _callbackMethod is not null;
    ...
    public void AccessCollection(IEnumerable collection, Action accessMethod, bool writeAccess)
    {
        if (_callbackMethod is not null)
        {
            ...
            if (!_callbackTarget.TryGetTarget(out object target))
            {
                throw new InvalidOperationException(string.Format(Strings.CollectionView_MissingSynchronizationCallback, collection));
            }

            // invoke the callback
            if (_callbackTarget == StaticWeakRef)
            {
                target = null;          // static method
            }

            object context = _context;
            if (_context is WeakReference<object> wrContext)
            {
                wrContext.TryGetTarget(out context);
            }
            _callbackMethod.Invoke(target, [collection, context, accessMethod, writeAccess]);
        }
    ...
    public bool IsAlive => (_callbackMethod is not null && _callbackTarget.TryGetTarget(out _)) || (_callbackMethod is null && _context is not null);

    public static readonly SynchronizationInfo None = new(null, null);

    private readonly object _context;
    private readonly MethodInfo _callbackMethod;
    private readonly WeakReference<object> _callbackTarget;

    private static readonly WeakReference<object> StaticWeakRef = new(StaticObject);
    private static readonly object StaticObject = new();
}

internal sealed class ViewManager : Dictionary<WeakRefKey, CollectionRecord>
{
    ...
    public CollectionRecord this[object o]
    {
        get
        {
            ...
            TryGetValue(new WeakRefKey(o, false), out CollectionRecord cr);
            return cr;
        }
    }
    ...
    {
        base.Add(new WeakRefKey(collection, true), cr);
    ...
    internal ViewRecord GetViewRecord(object collection, CollectionViewSource cvs, Type collectionViewType, bool createView, Func<object, object> GetSourceItem)
    {
        ...
        // underlying list.
        IList ilsList = null;
        if (collection is IListSource ils)
        {
            ilsList = ils.GetList();
        ...
        else if (collectionViewType == null)
        {
            ...
            if (collection is ICollectionViewFactory icvf)
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (collection is IEnumerable ie)
                    {
        ...
        else
        {
            ...
            {
                throw new ArgumentException(string.Format(Strings.CollectionView_WrongType, collectionViewType.Name));
            }
            ...
            try
            {
                icv = Activator.CreateInstance(collectionViewType, BindingFlags.CreateInstance, null, [arg], null) as ICollectionView;
            }
            catch (MissingMethodException e)
            {
                throw new ArgumentException(
                    string.Format(Strings.CollectionView_ViewTypeInsufficient, collectionViewType.Name, collection.GetType()), e);
            }
        }
        ...
        if (icv != null)
        {
            ...
            if (icv is not CollectionView cv)
            {
    ...
    private ViewRecord GetExistingView(object collection, CollectionViewSource cvs, Type collectionViewType, Func<object, object> GetSourceItem)
    {
        ...
        if (cv == null)
        {
            ...
            if (vt != null)
            {
                ...
                if (_inactiveViewTables.ContainsKey(vt))
                {
    ...
    internal bool Purge()
    {
        ...
        if (n > 0)
        {
            ...
            {
                ViewTable vt = keys[i];
                int expirationDate = _inactiveViewTables[vt];
                if (--expirationDate > 0)
                {
    ...
    private void ValidateViewType(CollectionView cv, Type collectionViewType)
    {
        if (collectionViewType != null)
        {
            ...
            // other view.
            Type cachedViewType = cv is not CollectionViewProxy cvp ? cv.GetType() : cvp.ProxiedView.GetType();

            if (cachedViewType != collectionViewType)
            {
                throw new ArgumentException(string.Format(Strings.CollectionView_NameTypeDuplicity, collectionViewType, cachedViewType));
            }
        }
    }

    private readonly Dictionary<ViewTable, int> _inactiveViewTables = new();
}
