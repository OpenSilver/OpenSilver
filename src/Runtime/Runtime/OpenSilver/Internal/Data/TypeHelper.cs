...
internal static class TypeHelper
{
    ...
    /// <returns></returns>
    private static PropertyInfo GetNestedProperty(Type parentType, string propertyPath, out Exception exception, ref object item)
    {
        exception = null;
        ...
        for (int i = 0; i < propertyNames.Count; i++)
        {
            ...
            object[] index = null;
            propertyInfo = GetPropertyOrIndexer(type, propertyNames[i], out index);
            ...
            type = GetNonNullableType(propertyInfo.PropertyType);
        }
    ...
    internal static Type GetNestedPropertyType(Type parentType, string propertyPath)
    {
        ...
        PropertyInfo propertyInfo = GetNestedProperty(parentType, propertyPath, out exception, ref item);
    ...
    internal static object GetNestedPropertyValue(object item, string propertyPath, Type propertyType, out Exception exception)
    {
        ...
        if (itemType != null)
        {
            PropertyInfo propertyInfo = GetNestedProperty(itemType, propertyPath, out exception, ref propertyValue);
    ...
    private static PropertyInfo GetPropertyOrIndexer(Type type, string propertyPath, out object[] index)
    {
        index = null;
        ...
        string propertyName = GetDefaultMemberName(type);
