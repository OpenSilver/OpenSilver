...
public class GridViewHeaderRowPresenter : GridViewRowPresenterBase
{
    ...
    /// <summary>
    /// Gets or sets the <see cref="Style"/> to use for the column headers.
    /// </summary>
    /// <returns>
    /// The <see cref="Style"/> to use for the column header container. The default is null.
    /// </returns>
    ...
    /// </summary>
    /// <returns>
    /// The <see cref="DataTemplate"/> that is used to display the column header content.
    /// The default is null.
    /// </returns>
    ...
    /// <summary>
    /// Gets or sets a <see cref="DataTemplateSelector"/> that provides logic that selects 
    /// the data template to use to display a column header.
    /// </summary>
    /// <returns>
    /// The <see cref="DataTemplateSelector"/> that chooses the <see cref="DataTemplate"/>
    /// to use to display each column header. The default is null.
    /// </returns>
    ...
    /// <summary>
    /// Gets or sets a composite string that specifies how to format the column headers
    /// if they are displayed as strings.
    /// </summary>
    /// <returns>
    /// A composite string that specifies how to format the column headers if they are 
    /// displayed as strings. The default is null.
    /// </returns>
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    ...
    /// </summary>
    /// <returns>
    /// true if columns can be moved by the drag-and-drop operation of a column header; otherwise,
    ...
    /// <summary>
    /// Gets or sets a <see cref="ContextMenu"/> for the column headers.
    /// </summary>
    /// <returns>
    /// The <see cref="ContextMenu"/> for the column header row. The default is null.
    /// </returns>
    ...
    /// </param>
    /// <returns>
    /// The required <see cref="Size"/> for the column header row.
    /// </returns>
    ...
    /// </param>
    /// <returns>
    /// The actual <see cref="Size"/> for the column header row.
    /// </returns>
    ...
    /// <summary>
    /// Provides class handling for the <see cref="UIElement.MouseLeftButtonDown"/> event that occurs 
    /// when the user presses the left mouse button inside a <see cref="GridViewColumnHeader"/>.
    /// </summary>
    /// <param name="e">
    /// The event data.
    /// </param>
    ...
    /// <summary>
    /// Provides class handling for the <see cref="UIElement.MouseLeftButtonUp"/> event that occurs 
    ...
    /// <summary>
    /// Provides class handling for the <see cref="UIElement.MouseMove"/> event that occurs when 
    ...
    /// <summary>
    /// Provides class handling for the <see cref="UIElement.LostMouseCapture"/> event for the 
    ...
    /// <summary>
    /// Creates an <see cref="AutomationPeer"/> object for the column header row.
    /// </summary>
    /// <returns>
    /// A <see cref="GridViewHeaderRowPresenterAutomationPeer"/> object for this column header row.
    /// </returns>
    ...
    private GridViewColumnHeader CreateAndInsertHeader(GridViewColumn column, int index)
    {
        ...
        if (header != null)
        {
            ...
            if (d != null)
            {
                ...
                UIElement headerAsVisual = d as UIElement;

                if (headerAsVisual != null)
                {
                    UIElement parent = VisualTreeHelper.GetParent(headerAsVisual) as UIElement;
        ...
        headerContainer.SetValueInternal(GridViewColumnHeader.ColumnPropertyKey, column);
    ...
    private void AddPaddingColumnHeader()
    {
        ...
        paddingHeader.SetValueInternal(GridViewColumnHeader.RolePropertyKey, GridViewColumnHeaderRole.Padding);
...
#if OPENSILVER
        // We need to make sure that the indicator is always visible when reordering columns.
        // We need to set a ZIndex because the indicator is added to the visual tree before
        // the column headers, which means its "natural" ZIndex is lower than the one of the
        // headers. We set it to int.MaxValue to cover the case where a user would define headers
        // explicitely and specify a ZIndex on the headers (note that in case the user sets
        // ZIndex to int.MaxValue too, the indicator will not be visible). In WPF, ZIndex is
        // ignored when the parent is not a Panel, so it is acceptable here to "ignore" headers
        // ZIndex by applying the maximum ZIndex value to the indicator because GridViewHeaderRowPresenter
        // is not derived from Panel.
        ...
        var template = new ControlTemplate(typeof(Separator))
        {
            Template = new CompiledTemplateContent(
                ...
                static (owner, context) =>
                {
                    ...
                    border.Background = new SolidColorBrush(Color.FromUInt32(0xFF000080));
                    return border;
                })
        };
    ...
    private void AddFloatingHeader(GridViewColumnHeader srcHeader)
    {
        ...
        {
            throw new ArgumentException(string.Format(Strings.ListView_MissingParameterlessConstructor, headerType), e);
        }

        Debug.Assert(header != null, "Cannot instantiate GridViewColumnHeader in AddFloatingHeader");

        header.IsInternalGenerated = true;
        header.SetValueInternal(GridViewColumnHeader.RolePropertyKey, GridViewColumnHeaderRole.Floating);
    ...
    private void UpdateFloatingHeader(GridViewColumnHeader srcHeader)
    {
        ...
        _floatingHeader.SetValueInternal(GridViewColumnHeader.ColumnPropertyKey, srcHeader.Column);
        ...
        if (srcHeader.Content is not UIElement)
        {
