    ...
    public class DataForm : Control
    {
        ...
        private static void OnCurrentItemPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (dataForm != null && !dataForm.AreHandlersSuspended())
            {
                ...
                {
                    dataForm.SetValueNoCallback(e.Property, e.OldValue);
                    throw new InvalidOperationException(string.Format(Globalization.CultureInfo.InvariantCulture, "Cannot change currency when an item has validation errors or it is being edited and {0} is false.  Set {1} to a {2} to manage currency instead.", "AutoCommit", "ItemsSource", "ICollectionView"));
                }
            }
        }
        ...
        private bool BeginEdit(bool startingNewEdit)
        {
            ...
            if (e.Cancel)
            {
                ...
                {
                    throw new InvalidOperationException(string.Format(Globalization.CultureInfo.InvariantCulture, "{0} cannot be canceled when {1} is set to true.", "BeginEdit", "AutoEdit"));
                }
            }
