...
/// <summary>
/// Arranges <see cref="ToolBar"/> items inside a <see cref="ToolBar"/>.
/// </summary>
public class ToolBarPanel : StackPanel
{
        ...
        for (int i = 0; i < _generatedItemsCollection.Count; i++)
        {
            ...
            if (asNeededMode == asNeededPass)
            {
                ...
                if ((overflowMode != OverflowMode.Always) && !sendToOverflow)
                {
                    ...
                    EnsureMeasureable(child, overflowPanel, children, childrenIndex, ref visualParent, ref childrenCount);
    ...
    private void EnsureMeasureable(
        UIElement child,
        ToolBarOverflowPanel overflowPanel,
        UIElementCollection children,
        int childrenIndex,
        ref DependencyObject visualParent,
        ref int childrenCount)
    {
        if (child.OuterDiv.IsConnected)
        {
            return;
        }
    ...
    /// The measurement constraints; a <see cref="ToolBarPanel"/> cannot return a size larger 
    /// than the constraint.
    /// </param>
    /// <returns>
    ...
    protected override Size MeasureOverride(Size constraint)
    {
        ...
        if (IsItemsHost)
        {
            ...
            toolbar?.SetValueInternal(ToolBar.HasOverflowItemsPropertyKey, hasAlwaysOverflowItems || hasAsNeededOverflowItems);
        }
        else
        {
    ...
    /// <summary>
    /// Arranges and sizes <see cref="ToolBar"/> items inside a <see cref="ToolBarPanel"/>.
    /// </summary>
    /// <param name="arrangeSize">
    /// The size that the <see cref="ToolBarPanel"/> assumes to position its children.
    /// </param>
    /// <returns>
    ...
    internal override void OnPreApplyTemplate()
    {
        ...
        if (TemplatedParent is ToolBar && HasDefaultValue(OrientationProperty))
        {
