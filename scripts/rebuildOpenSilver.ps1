param (
    $OS_VERSION = $(if ($null -eq $env:OS_VERSION) { "2.2.0" } else { $env:OS_VERSION }),
    ...
    $OS_SIMULATOR_VERSION = $(if ($null -eq $env:OS_SIMULATOR_VERSION) { "2.2.0" } else { $env:OS_SIMULATOR_VERSION }),
    $MSBUILD = "msbuild.exe",
