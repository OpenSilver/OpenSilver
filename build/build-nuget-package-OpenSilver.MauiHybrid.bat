...
ECHO.
msbuild %BUILD_DIR%\slnf\OpenSilver.MauiHybrid.slnf -p:Configuration=%CFG%;OpenSilverBuildUUID=%BUILD_UUID% -clp:ErrorsOnly -restore

ECHO.
ECHO %ESC%[95mPacking %ESC%[0mOpenSilver.MauiHybrid %ESC%[95mNuGet package%ESC%[0m
ECHO.
%BUILD_DIR%\nuget.exe pack %BUILD_DIR%\nuspec\OpenSilver.MauiHybrid.nuspec -OutputDirectory "%BUILD_DIR%\output\OpenSilver" -Properties "PackageVersion=%PackageVersion%;Configuration=%CFG%;OpenSilverBuildUUID=%BUILD_UUID%;RepositoryUrl=https://github.com/OpenSilver/OpenSilver"
